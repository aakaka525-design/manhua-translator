...
- **新增词库项（明确纠错）**：
  - `부들` -> `颤抖`
  - `무들` -> `颤抖`
  - `헤이뉴트드`、`탈간잘자리` 默认走韩文音译（romanization），不强行对应中文拟声词。
- **低风险启发式**（满足全部条件才判 SFX，且引入强信号要求）：
  1. 去除末尾标点后，韩文长度 <= 4 时，也必须满足强信号（见第 5 点）才判；
  2. 韩文长度在 5-6 时仅在重复音节模式命中时判；
  2. 韩文字母占比 >= 0.8，
  3. 不含空格，
  4. 允许末尾 `!/?/…` 等标点，
  5. 强信号：重复音节模式（如 `쿵쿵` / `근근근`）或末尾感叹号；
     若无强信号，则不因长度而判定 SFX。
...
- 当目标语言为中文且 `english_ratio >= 0.35`（阈值已确定），触发一次回退翻译（优先使用原文输入）。
