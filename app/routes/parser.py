    ...
    if "page" in segments:
        ...
        if page_index + 1 < len(segments):
            ...
            if page_value.isdigit():
                page = int(page_value)
                del segments[page_index : page_index + 2]
    cleaned_path = "/" + "/".join(segments)
    if not cleaned_path.endswith("/"):
        cleaned_path += "/"
    query = parse_qs(parsed.query)
    orderby_values = query.get("m_orderby")
    orderby = orderby_values[0] if orderby_values else None
    return cleaned_path, page, orderby
...
async def _list_recognized_catalog(
    ...
    if not site or not base_url:
        return [], ["Unsupported catalog host"]
    path, page, orderby = _parse_catalog_url(url)
    request = ScraperCatalogRequest(
        base_url=base_url,
    ...
    engine, _ = scraper_routes._build_engine(request, Path(settings.data_dir))
    try:
        result = await engine.list_catalog(page=page, orderby=orderby, path=path)
    except Exception as exc:  # noqa: BLE001
        return [], [f"Catalog fetch failed: {exc}"]
    results_list, _ = result
    items = [
        {
            "id": item.id,
            "title": item.title,
