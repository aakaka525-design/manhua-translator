...
class ToonGodScraper(GenericPlaywrightScraper):
    def __init__(
        self,
        ...
        super().__init__(
            ...
            search_path="/?s={keyword}&post_type=webtoon",
            chapter_path="/webtoon/{manga_id}",
            reader_path="/webtoon/{manga_id}/{chapter_id}/",
            selectors=defaults,
        )
    ...
    async def search_manga(self, keyword: str):
        ...
        if slug:
            direct_url = urljoin(self.config.base_url, f"/webtoon/{slug}/")
        ...
        for path in search_paths:
            if path in seen:
                continue
            seen.add(path)
            url = urljoin(self.config.base_url, path.format(keyword=quote(keyword)))
